...
<article class="builds-page reader"><a href="/builds" class="back-link">← All builds</a>
  <header class="guide-header"><div class="guide-badges"><a href={`/builds?characterClass=${build.characterClass}`}>{build.characterClass}</a><span>{build.category}</span><span>{build.budget}</span>{#if build.patch}<span>{build.patch}</span>{/if}</div>
    ...
    <div class="guide-actions"><Button type="button" color="alternative" size="sm" onclick={share}>Copy build link ↗</Button>{#if isOwner}<Button href={`/builds/${build.id}/edit`} color="alternative" size="sm">Edit build</Button>{/if}<Button href="#discussion" color="alternative" size="sm">Join the discussion</Button></div>
  ...
  <section class="community-panel panel" id="community"><div><p class="eyebrow">PLAYER FEEDBACK</p><h2>How does this build play?</h2><p>Rate the guide after trying it. Your feedback helps other players choose.</p></div>
  ...
  <section class="discussion" id="discussion"><div class="discussion-heading"><h2>Discussion <span>{comments.total}</span></h2><p>Ask questions, share your results, and help refine the build.</p></div>
    {#if $authState.user}<form class="panel" onsubmit={postComment}><label for="build-comment">Your comment</label>{#if replyTo}<p class="replying">Replying to {replyTo.authorName}: {replyTo.body.slice(0,120)} <button type="button" onclick={() => replyTo = null}>Cancel reply</button></p>{/if}<textarea class="field" id="build-comment" bind:value={commentBody} maxlength="2000" rows="4" required placeholder="What worked for you? What would you like to know?"></textarea><div class="comment-form-footer"><small>{commentBody.length}/2000 · Be constructive and respectful.</small><Button type="submit" color="red" size="sm" disabled={busy || !commentBody.trim()}>Post comment</Button></div></form>
    {:else}<div class="sign-in-panel panel"><p>Have a question or a suggestion?</p><Button href={signInUrl} color="alternative" size="sm">Sign in to comment</Button></div>{/if}
    ...
    </article>{/each}</div><nav class="pager" aria-label="Discussion pages"><Button color="alternative" size="sm" disabled={commentsBusy || comments.skip === 0} onclick={() => loadComments(Math.max(0,comments.skip-30))}>← Newer</Button><span>{comments.skip+1}–{Math.min(comments.skip+30,comments.total)} of {comments.total}</span><Button color="alternative" size="sm" disabled={commentsBusy || comments.skip+30 >= comments.total} onclick={() => loadComments(comments.skip+30)}>Older →</Button></nav>{/if}
  </section>
</article>
<style>
  .guide-header{padding:2rem 0 2.2rem;}.guide-badges,.guide-tags{display:flex;gap:.6rem;flex-wrap:wrap;}.guide-badges > *{padding:.4rem .6rem;border:1px solid #b29b603d;border-radius:3px;color:var(--color-parchment-200);font-size:12px; line-height:1.5;}.reader .build-title{max-width:1000px;font-size:clamp(1.8rem,4vw,3rem);overflow-wrap:anywhere;}.byline{display:flex;flex-wrap:wrap;gap:.8rem;align-items:center;font-size:12px; line-height:1.5;color:var(--color-parchment-300);margin:1.5rem 0;}.byline strong{color:var(--color-parchment-200);}.byline a{color:var(--color-parchment-200);}.author-avatar{display:grid;place-items:center;flex:none;width:27px;height:27px;background:#b19a6010;border:1px solid #b19a6038;border-radius:50%;font-size:12px; line-height:1.5;color:var(--color-parchment-200);}.guide-actions{display:flex;flex-wrap:wrap;gap:.6rem;}.guide-tags{margin-top:1.2rem;font-size:12px; line-height:1.5;color:var(--color-parchment-300);}.reader-notice{padding:1rem;background:#8ba37810;border:1px solid #8ba37840;border-radius:4px;font-size:13px; line-height:1.7;overflow-wrap:anywhere;}
  .community-panel{display:flex;justify-content:space-between;gap:2rem;padding:2rem;margin:3rem 0;scroll-margin-top:90px;}.community-panel h2,.discussion h2{font:1.5rem var(--font-display);color:var(--color-parchment-50);margin:.7rem 0;}.community-panel p:not(.eyebrow),.discussion-heading p{font-size:13px; line-height:1.8;color:var(--color-parchment-300);}.rating-box{text-align:right;min-width:215px;}.rating-summary{font-size:20px; line-height:1.5;color:var(--color-parchment-200);}.rating-summary small{display:block;font-size:12px; line-height:1.5;color:var(--color-parchment-300);margin:.5rem 0;}.stars{display:flex;justify-content:flex-end;gap:.4rem;}.stars button{font-size:28px; line-height:1.5;color:var(--color-parchment-300);padding:.2rem;cursor:pointer;}.stars button.chosen,.stars button:hover{color:var(--color-parchment-200);}.text-button{display:block;color:var(--color-parchment-200);font-size:12px; line-height:1.5;margin-top:.7rem;text-decoration:underline;}.rating-box .text-button{margin-left:auto;}
  .discussion{max-width:900px;margin:0 auto;scroll-margin-top:90px;}.discussion-heading{margin-bottom:1.5rem;}.discussion h2 span{font-size:13px; line-height:1.5;color:var(--color-parchment-300);}.discussion form{padding:1.3rem;margin-bottom:2rem;}.discussion label{font-size:12px; line-height:1.5;color:var(--color-parchment-300);}.discussion textarea{display:block;width:100%;min-width:0;margin:.8rem 0; resize:vertical;}.comment-form-footer{display:flex;justify-content:space-between;gap:1rem;align-items:center;flex-wrap:wrap;}.comment-form-footer small{font-size:12px; line-height:1.5;color:var(--color-parchment-300);}.sign-in-panel{display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;padding:1.5rem;border-radius:5px; margin-bottom:2rem;}
  .comment{padding:1.5rem 0;border-bottom:1px solid #ffffff10;}.comment-meta{display:flex;align-items:center;gap:.7rem;flex-wrap:wrap;font-size:12px; line-height:1.5;color:var(--color-parchment-200);}.comment-meta time{margin-left:auto;font-size:12px;color:var(--color-parchment-300);}.author-badge{font-size:12px;border:1px solid #b19a6040;padding:.2rem .4rem;border-radius:3px;color:var(--color-parchment-200);}.comment-body{font-size:14px; line-height:1.8;color:var(--color-parchment-200);white-space:pre-wrap;overflow-wrap:anywhere;margin:1rem 0;}.comment-actions{display:flex;gap:1rem;font-size:12px; line-height:1.5;color:var(--color-parchment-300);}.no-comments{padding:2rem 0;font-size:13px; line-height:1.5;color:var(--color-parchment-300);}.replying,.reply-context{font-size:12px; line-height:1.6;color:var(--color-parchment-300);padding:.7rem 0;overflow-wrap:anywhere;}.replying button{text-decoration:underline;margin-left:.5rem;}.remove-confirm{border:1px solid #b8705450;padding:1rem;margin-top:.8rem;color:var(--color-parchment-200);font-size:12px; line-height:1.5;}.remove-confirm button{margin-left:1rem;text-decoration:underline;}
